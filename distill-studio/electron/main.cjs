...
const PREFERRED_PORT = Number(process.env.PORT || 8877);
const ROOT = path.join(__dirname, "..");
const USER_DATA =
  process.env.DISTILL_ELECTRON_USER_DATA ||
  path.join(process.env.USERPROFILE || ROOT, ".distill-studio-electron");
...
function startServer(port) {
  ...
  if (fs.existsSync(bundled)) {
    if (app.isPackaged) {
      ...
      // Prefer system node if available for clearer logs in dev
      // (ELECTRON_RUN_AS_NODE still works).
    }
    spawnedByUs = true;
    return;
  }
...
async function createWindow() {
  ...
  if (!picked.reuse) {
    ...
    console.log(`复用已运行的 Distill Studio 服务 :${usedPort}`);
  }
  ...
  win = new BrowserWindow({
    width: 1200,
    height: 800,
    title: `Distill Studio${usedPort !== PREFERRED_PORT ? ` (:${usedPort})` : ""}`,
...
app.whenReady().then(createWindow).catch(async (err) => {
  ...
  dialog.showErrorBox(
    "Distill Studio 启动失败",
    `${err && err.message ? err.message : String(err)}\n\n可尝试结束占用 8877 的进程后重试，或设置环境变量 PORT=8878`,
  );
  app.quit();
});
